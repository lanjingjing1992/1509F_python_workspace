...
class Test:
    ...
    def _login(self,name,passwd):
        ...
        if name in self.p.map.keys():#用户名存在的
            if passwd == self.p.map[name]:#密码一致
                print('登陆成功！！！')
                self._calc()
                return
            else:
            ...
            print('注册')#像字典中添加键值对
            self.p.map.update({name:passwd})
            self._login()

    def _calc(self):#计算商品总价 并且打印  单价*数量
    ...
    def writeAndRead(self,n):#用来读写文件
        os.chdir('/users/lanjingjing/python/')
